...
from modules.layer import simple_mlp
...
def main():
    ...
    ds_fn = CIFAR10 if fashion else MNIST
...
def training_loop(ddpm, loader, n_epochs, opt, device, display=False, store_path="ddpm_model.pt"):
    ...
    for epoch in tqdm(range(n_epochs), desc=f"Training progress", colour="#00ff00"):
        ...
        for step, batch in enumerate(tqdm(loader, leave=False, desc=f"Epoch {epoch + 1}/{n_epochs}", colour="#005500")):
            ...
            epsilon_theta = ddpm.regenerate(noisy_imgs, t.reshape(n, -1))
...
def generate_new_images(ddpm, n_samples=16, device=None, frames_per_gif=100, gif_name="sampling.gif", c=1, h=28, w=28):
    ...
    with torch.no_grad():
        ...
        for idx, t in enumerate(list(range(ddpm.n_steps))[::-1]):
            ...
            eta_theta = ddpm.regenerate(x, time_tensor)
...
class MyUNet(nn.Module):
    def __init__(self, n_steps=1000, time_emb_dim=100, device=None):
        ...
        self.b1 = nn.Sequential(
            LayerNorm((1, 28, 28), 1, 10),
            LayerNorm((10, 28, 28), 10, 10),
            LayerNorm((10, 28, 28), 10, 10)
        )
        ...
        self.b2 = nn.Sequential(
            LayerNorm((10, 14, 14), 10, 20),
            LayerNorm((20, 14, 14), 20, 20),
            LayerNorm((20, 14, 14), 20, 20)
        )
        ...
        self.b3 = nn.Sequential(
            LayerNorm((20, 7, 7), 20, 40),
            LayerNorm((40, 7, 7), 40, 40),
            LayerNorm((40, 7, 7), 40, 40)
        )
        ...
        self.b_mid = nn.Sequential(
            LayerNorm((40, 3, 3), 40, 20),
            LayerNorm((20, 3, 3), 20, 20),
            LayerNorm((20, 3, 3), 20, 40)
        )
        ...
        self.b4 = nn.Sequential(
            LayerNorm((80, 7, 7), 80, 40),
            LayerNorm((40, 7, 7), 40, 20),
            LayerNorm((20, 7, 7), 20, 20)
        )

        self.up2 = nn.ConvTranspose2d(20, 20, 4, 2, 1)
        self.te5 = simple_mlp(time_emb_dim, 40)
        self.b5 = nn.Sequential(
            LayerNorm((40, 14, 14), 40, 20),
            LayerNorm((20, 14, 14), 20, 10),
            LayerNorm((10, 14, 14), 10, 10)
        )

        self.up3 = nn.ConvTranspose2d(10, 10, 4, 2, 1)
        self.te_out = simple_mlp(time_emb_dim, 20)
        self.b_out = nn.Sequential(
            LayerNorm((20, 28, 28), 20, 10),
            LayerNorm((10, 28, 28), 10, 10),
            LayerNorm((10, 28, 28), 10, 10, normalize=False)
        )
