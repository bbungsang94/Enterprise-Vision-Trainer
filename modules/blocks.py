...
class TransposedDoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None, residual=False):
        super().__init__()
        self.residual = residual
        if not mid_channels:
            mid_channels = out_channels
        self.double_conv = nn.Sequential(
            nn.ConvTranspose2d(in_channels, mid_channels, kernel_size=3, stride=2, padding=1, bias=False),
            nn.GroupNorm(1, mid_channels),
            nn.SiLU(),
            nn.ConvTranspose2d(mid_channels, out_channels, kernel_size=3, stride=2, padding=1, bias=False),
            nn.GroupNorm(1, out_channels),
        )
