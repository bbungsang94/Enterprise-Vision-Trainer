...
g_all.load_state_dict(torch.load('./pretrained/karras2019stylegan-ffhq-1024x1024.for_g_all.pt'))
...
for i in range(len(fixed_latents) - 1):
    latents.append(
        fixed_latents[i] + (fixed_latents[i + 1] - fixed_latents[i]) * torch.arange(0, 1, 0.1, device=device).unsqueeze(
            1))
